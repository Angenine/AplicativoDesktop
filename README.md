...
O sistema foi desenhado para atender aos requisitos do um projeto, com foco em:
  ...
  * **Persistência de Dados:** Conexão e operação com um banco de dados relacional (**MySQL**) para persistência total dos dados.
...
## Funcionalidades
  ...
  * **Persistência em Banco de Dados:** O acervo é carregado do MySQL ao iniciar, e todas as alterações (adicionar, atualizar, remover) são salvas no banco de dados em tempo real.
...
O código-fonte está organizado dentro do pacote `colecao/`:
  ...
  * **`repository.py` (Camada de Persistência):**
    Responsável pela comunicação com o banco de dados **MySQL**. Traduz os objetos Python para comandos SQL (INSERT, UPDATE, SELECT, DELETE) e vice-versa. É a única camada que "fala" com o banco.
  * **`db_config.py` (Configuração):**
    Arquivo separado que armazena as credenciais (host, usuário, senha) para a conexão com o banco de dados MySQL, mantendo-as fora do código principal.
...
**Pré-requisitos:**
  ...
  * Um servidor **MySQL** local em execução (ex: via MySQL Workbench).
  * A biblioteca Python `mysql-connector-python`.
...
4.  **Configuração do Banco de Dados:**
    a. Abra seu **MySQL Workbench** e crie um novo schema (banco de dados) chamado `db_colecao`.
    b. Dentro do schema `db_colecao`, execute o script SQL abaixo para criar a tabela `itens_colecao`:

    ```sql
    CREATE TABLE IF NOT EXISTS itens_colecao (
    ...
    c. Edite o arquivo `colecao/db_config.py` com suas credenciais do MySQL (host, usuário, senha).
6.  Execute o ponto de entrada `main.py`:
...
7.  O menu interativo será iniciado no seu terminal.
...
│   ├── repository.py         # Camada de Persistência (MySQL)
│   ├── db_config.py          # Configuração de credenciais do BD
│   ├── service.py            # Camada de Lógica de Negócio
